...
### R에서의 정규 표현식(Regular Expression, Regex)이란 ?
...
### 정규 표현식 기본 패턴 (아래의 예시와 매칭이 되지 않아 수정이 필요합니다)
...
#| code-line-numbers: true
      2018-12-05   Thyroid_CT cN1b
      2019-01-22   TTcND_Lt.SND LGH
#1. PTC(Lt_x2 Rt_x1) T1b(1.1cm ETE+ BRAF-) N1b(1/2, 7/22 ENE+) M1(Lung_focal)
...
-   op_line 항목 내에서는 숫자로 시작하지 않으면 `risk_data_op_line_error` 데이터셋으로 별도로 구분되어 보이도록 하였습니다.
...
```
-   `tstrsplit`함수를 사용하였고, "\\s+" 정규식을 사용하여 공백이 한번이상 반복되면 분리하였고 첫번째 `df$cN_date`라는 날짜로 하였습니다.
-   `tstrsplit`함수가 분리해 주는 두 번째는 영상검사종류로 하였습니다.
-   세 번째는 clinical N stage로 하였습니다.
-   앞서의 예시에는 없었지만 PCNA(+)가 있다면 이는 cN stage에 대해 PCNA가 시행되었고 그 결과가 양성임을 저자가 정의한 것입니다. 이를 의미합므로 한 추가 정보로 "Positive"로 파생하였습니다.
    -   정규식에서 괄호는 그룹핑을 의미합니다. 따라서 괄호 그 자체를 표현하기 위해서는 escape 문자인 역슬래시를 두 번 사용해서 `PCNA \\(\\+\\)`를 사용하였습니다.
