...
-   R에서의 regular expression의 활용에 대한 이해를 돕기 위해 실제 사용 예시를 나열합니다.

### 텍스트예시

-   다음은 갑상선암 외래 진료시 EMR의 일부를 보여 드립니다.
...
-   저자는 이를 파싱해서 데이터베이스로 만드는 중입니다. 그래서 사전에 미리 정한 규칙에 따라 EMR 텍스트들을 작성하였습니다 (물론 규칙이 변하기도 하고, 아직 적용되지 않은 환자들도 있고 많은 변수들이 자리잡고 있기는 합니다.) 그러면 EMR 텍스트데이터가 어떤 규칙으로 작성되었는지를 설명드리겠습니다.

    -   1번줄은 수술전 병기설정을 위해 시행된 영상검사를 위한 줄입니다.
        -   날짜와 영상검사종류와 clinical N stage를 기술하였습니다.
        ...
        -   만약 수술 전 영상검사가 없거나 정보를 구할 수 없다면 이 줄은 없습니다.
        -   만약 수술 전 PETCT 영상검사가 있었다면 `&`로 구분하고 날짜와 PETCT를 기술하였습니다. 그리고 primary 지만 clinical N stage가 없다면 이 줄은 없습니다.
    -   2번줄은 수술종류와 수술의사를 기술하였습니다. 역시 공백으로 구분되어 있으며
    -   3번줄은 staging을 기술하였습니다.
        -   `#1.`은 첫번째 상병으로 저에게는 갑상선암입니다.
        -   공백을 두고 histologic subtype을 사전에 정의한 알파벳으로 시작하는 약자로 기술하였습니다.
        ...
        -   공백을 두고 pathologic T staging을 기술한 후 괄호안에 관련예후인자들을 기술하였습니다.
            -   괄호안에서는 N1a와 N1b 정보를 `,`로 구분했습니다.
                -   N1a에서는 몇개의 림프절을 절제해서 몇개가 양성이었는지를 기술했습니다. 만약 extranodal extension이 있다면 공백을 두고 `ENE+`를 추가했습니다.
    ...
    -   참고로 위하여 추가 설명을 하자면 EMR을 데이테베이스로 하기 위해서 저자가 선택한 전략을 `관심환자`기능을 사용하는 것이었습니다.
        ...
        -   예를 들어서,
            ...
            -   실제로 저자가 구현한 것은 이보다 더 복잡하지만 설명을 위해서 단순화한 것임을 참고하시길 바랍니다.
            
            
### strsplit 함수
character vector 만을 인자로 받는 base 함수입니다. character type의 data.frame  열로 내부적으로는 character vertor로 처리되므로 인자로 받아서 결과를 반환할 수 있지만 이는 리스트 형식임을 잘 이해해야 합니다. 이는 구체적인 예시로 통하면 이해에 좀 더 도움이 됩니다.
...
-   위에서 생성된 df 데이터프레임의 등록일을 strsplit 함수로써 "-"을 기준으로 분리한 결과를 df의 new란 열로 파생시키는 코드의 결과를 확인해 보시길 바랍니다.
...
-   df$new 열에는 연도와 월과 일이 분리되어 리스트형태로 파생되어 있습니다. 문제는 간단한 코드로 연도만 분리하여 파생시키키가 어렵다는 것입니다.
-   strsplit 결과 리스트에서 첫번째 환자의 결과들만 접근할려면 리스트의 인덱싱 방법에 따라 [[1]]을 부연하면 됩니다.
-   strsplit 결과 리스트에서 첫번째 환자의 첫번째 결과만 접근하려면 리스트의 인덱싱 방법에 따라 [[1]][[1]]을 부연하면 됩니다.
-   하지만 모든 환자의 첫번째 결과들 모두 접근하는 방법은 자체적으로 지원되지 않습니다.
-   모든 환자의 첫번째 결과들 모두 접근하기 위해 for 문법을 적용하면 아래와 같습니다.
...
- R의 장점 중의 하나인 list의 각 요소에 같은 함수를 적용할 수 있는 sapply 함수를 사용하면 for 문법을 사용하지 않고도 같은 결과를 얻을 수 있습니다.
...
```
-   tstrsplit 함수는 strsplit 함수와 유사하게 작동하지만 결과를 리스트가 아닌 data.table로 반환합니다. 이는 data.table의 장점을 살려서 빠르게 처리할 수 있습니다.
...
### tstrsplit 함수
-   이 함수는 data.table에 포함된 함수로 strsplit 함수와 유사하게 작동합니다.
...
-   이 data.table을 인자로 받아서 특기사항의 한줄씩 분리하고 새로운 항목명을 부여한여 만든 새로운 data.table을 반환하는 함수를 만들어 진행하였습니다.
...
-   특기사항에 줄바꿈(`"\n"`)이 몇개가 포함되었는지를 알아내기 위해 str_count란 함수를 사용하였고 그 결과로 새로운 `line_count`열을 만들었습니다. data.table을 사용하여 문법이 다소 어렵지만 데이터가 큰 경우 속도가 빨라 장점이 많습니다.
...
-   특기사항을 줄단위로 분리하기 위해서 `tstrsplit`함수를 사용하였습니다. 이 함수는 `data.table`에 포함된 함수로 `strsplit`함수와 유사하게 작동합니다.
-   3줄일 때는 cN_line열까지 만들고, 2줄일 때는 op_line열과 stage_line열을 만들도록 하였습니다.
...
-   cN_line열을 공백으로 분리하여 새로운 열을 만들어 반환하는 함수를 만들었습니다.
...
-   첫 번째(=`cN_line`) 줄에 `&`가 있는 경우는 이후 수술 전 PETCT검사 내용이 기술괴어 있으므로 `&` 이전의 임의의 모든문자와 &와 이후의 반복되는 공백을 제거하하기 위해 `.*&\\s=` 정규식을 사용하였고 이를 `df$PETCT_part`로 파생했습니다.
